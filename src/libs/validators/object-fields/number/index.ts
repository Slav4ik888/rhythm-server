 ...
 */
export const validateNumber = (data: ContainsField, field: string, options: ValidateNumberOptions = {}): Validation => {
  const
  ...
  if (required) {
    ...
    if (! data || isNotHasField(data, field)) return getValidResult()
  }
  
  // Check min max
  if (isNotUndefined(min) && value < (min as unknown as number)) return getValidResult({ [field]: ErrorText.NUM_LESS_THAN })
  if (isNotUndefined(max) && value > (max as unknown as number)) return getValidResult({ [field]: ErrorText.NUM_MORE_THAN })
